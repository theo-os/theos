...
def compile_context(ctx: AnalysisContext, binary: bool = False) -> CompileContext:
    ...
    exec_is_windows = ctx.attrs._exec_os_type[OsLookup].script == ScriptLanguage("bat")
...
def _linker(
    ...
    binary: bool = False) -> (cmd_args, list):
    pre_args = []

    def _extend_args(dst: list, xs):
        if xs == None:
            return
        if type(xs) == type([]):
            for x in xs:
    ...
    _extend_args(pre_args, linker_info.linker_flags or [])
    ...
    _extend_args(pre_args, linker_info.binary_linker_flags if binary else [])
    _extend_args(pre_args, ctx.attrs._rust_toolchain[RustToolchainInfo].linker_flags)
    _extend_args(pre_args, ctx.attrs.linker_flags)

    exec_is_windows = ctx.attrs._exec_os_type[OsLookup].script == ScriptLanguage("bat")
    linker_str = str(linker_info.linker).strip().lower()

    # On Windows, using a `.bat` wrapper forces Rust to invoke `cmd.exe /c`.
    # If the toolchain's linker is already a standalone executable (e.g.
    # `lld-link` / `link.exe`) and has no extra argv, pass it directly to rustc
    # and feed any needed args through `-Clink-arg`, which avoids cmd.
    is_single_exe = (" " not in linker_str and "\t" not in linker_str and linker_str != "")
    is_standalone_linker = (
        linker_str.endswith("lld-link") or
        linker_str.endswith("lld-link.exe") or
        linker_str.endswith("link.exe")
    )

    if exec_is_windows and is_single_exe and is_standalone_linker:
        filtered_pre_args = []
        for a in pre_args:
            if type(a) == type(""):
                if a.startswith("-fuse-ld=") or a == "-flavor" or a.startswith("-flavor="):
                    continue
            filtered_pre_args.append(a)
        return linker_info.linker, filtered_pre_args

    # Default behavior: bundle linker + pre-args into a single invokable script.
    # Note that on Windows this is a `.bat` and will be invoked via cmd.exe.
    return cmd_script(
        actions = ctx.actions,
        name = "linker_wrapper",
        cmd = cmd_args(
